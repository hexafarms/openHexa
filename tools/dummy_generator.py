...
def dummy_generator(W, H, typ):
    ...
    if typ == "Barrel":
        dummy = cv2.circle(
            dummy, (w, h), int(max(h, w)), (intensity, intensity, intensity), -1
        )

    elif typ == "Pincushion":
        dummy = cv2.circle(
            dummy, (w, -h), int(h * 1.2), (intensity, intensity, intensity), -1
        )
        dummy = cv2.circle(
            dummy, (-w, h), int(w * 1.1), (intensity, intensity, intensity), -1
        )
    else:
